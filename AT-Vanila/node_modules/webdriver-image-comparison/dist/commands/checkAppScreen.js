 ...
 */
export default async function checkAppScreen(methods, instanceData, folders, tag, checkScreenOptions, isNativeContext) {
    // 1. Set some vars
    ...
    const { devicePixelRatio, fileName, isLandscape, } = await saveAppScreen(methods, instanceData, folders, tag, saveAppScreenOptions, isNativeContext);
    // 3. Determine the ignore regions
    const { ignore } = checkScreenOptions.method;
    const ignoreRegions = await determineIgnoreRegions(ignore || [], getElementRect);
