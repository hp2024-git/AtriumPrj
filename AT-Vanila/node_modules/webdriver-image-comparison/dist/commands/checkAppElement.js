 ...
 */
export default async function checkAppElement(methods, instanceData, folders, element, tag, checkElementOptions, isNativeContext) {
    // 1. Set some vars
    const { isMobile } = instanceData;
    const { executor } = methods;
    // 2. Save the element and return the data
    const { devicePixelRatio, fileName, isLandscape } = await saveAppElement(methods, instanceData, folders, element, tag, checkElementOptions, isNativeContext);
