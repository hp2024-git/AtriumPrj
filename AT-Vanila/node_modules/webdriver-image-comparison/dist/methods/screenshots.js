...
export async function getBase64FullPageScreenshotsData(takeScreenshot, executor, options) {
    const { addressBarShadowPadding, devicePixelRatio, fullPageScrollTimeout, hideAfterFirstScroll, innerHeight, isAndroid, isAndroidNativeWebScreenshot, isAndroidChromeDriverScreenshot, isHybridApp, isIOS, isLandscape, screenHeight, screenWidth, toolBarShadowPadding, } = options;
    ...
    if (isAndroid && isAndroidNativeWebScreenshot) {
        ...
        const { safeArea, screenHeight, screenWidth, sideBar: { width: sideBarWidth }, statusAddressBar: { height: statusAddressBarHeight }, } = (await executor(getAndroidStatusAddressToolBarOffsets, ANDROID_OFFSETS, { isHybridApp, isLandscape }));
        ...
        const { safeArea, screenHeight, screenWidth, sideBar: { width: sideBarWidth }, statusAddressBar: { height: statusAddressBarHeight }, toolBar: { y: iosHomeBarY }, } = await executor(getIosStatusAddressToolBarOffsets, IOS_OFFSETS, isLandscape);
...
export async function getFullPageScreenshotsDataNativeMobile(takeScreenshot, executor, options) {
    ...
    const { addressBarShadowPadding, devicePixelRatio, fullPageScrollTimeout, hideAfterFirstScroll, innerHeight, iosHomeBarY, safeArea, isLandscape, statusAddressBarHeight, screenHeight, sideBarWidth, toolBarShadowPadding, } = options;
    const iosViewportHeight = innerHeight -
        addressBarShadowPadding -
        toolBarShadowPadding -
 ...
 */
export async function takeWebElementScreenshot({ devicePixelRatio, element, executor, fallback = false, innerHeight, isAndroidNativeWebScreenshot, isAndroid, isIOS, isLandscape, screenShot, takeElementScreenshot, }) {
    if (isIOS || fallback) {
